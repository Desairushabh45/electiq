...
export default function Navbar({ screen, nav }) {
  ...
  return (
    <header role="banner" className="fixed top-0 left-0 right-0 z-50 bg-white border-b border-slate-100 shadow-sm">
      {/* Tricolor top stripe */}
      ...
      <nav role="navigation" aria-label="Main Navigation" className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
        ...
        <button onClick={() => { nav(SCREENS.HOME); setOpen(false); }}
          ...
          className="flex items-center gap-2 font-extrabold text-xl focus:outline-none focus:ring-2 focus:ring-blue-500 rounded-lg p-1"
        ...
        <div className="hidden md:flex items-center gap-1">
          {LINKS.slice(1).map(link => (
            <button key={link.screen} onClick={() => nav(link.screen)}
              ...
              className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                screen === link.screen
          ...
          <button
            ...
            className="ml-3 px-4 py-2 rounded-lg text-sm font-bold text-white transition-all hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-blue-500"
        ...
        {/* Mobile hamburger */}
        <button className="md:hidden p-2 rounded-lg text-slate-600 hover:bg-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500"
      ...
      {/* Mobile menu */}
      {open && (
        <div id="mobile-menu" className="md:hidden bg-white border-t border-slate-100 px-4 py-3 flex flex-col gap-1 shadow-lg">
          {LINKS.map(link => (
            <button key={link.screen}
              onClick={() => { nav(link.screen); setOpen(false); }}
              aria-current={screen === link.screen ? 'page' : undefined}
              aria-label={`Go to ${link.label}`}
              className={`text-left px-4 py-3 rounded-xl text-sm font-semibold transition-all focus:outline-none focus:ring-2 focus:ring-blue-500 ${
          ...
          <button
            ...
            aria-label="Open AI Assistant"
            className="mt-1 py-3 rounded-xl text-sm font-bold text-white text-center focus:outline-none focus:ring-2 focus:ring-blue-500"
            style={{ background: '#FF9933' }}>
